...
void app_main(void)
{
    ...
    // v0.4.39 USB-HID-REPORT-DESCRIPTOR-FULL with live input report polling:
    // On NEW_DEV, open/read descriptors/claim HID interface, then task submits one 64-byte HID report descriptor request.
    // Waits 30s, captures full 515-byte HID report descriptor, dumps/parses NUT map, then periodically polls HID Input Report (0x07) for live telemetry. Exact allocation; no MQTT.
