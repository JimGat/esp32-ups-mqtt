...
static volatile int64_t descriptor_ready_at_ms = 0;
...
ESP_LOGI(TAG, "📊 TELEMETRY: 0x07 Status Flags = 0x%04X (Power=%s) [RAW: %s]", 
         flags, on_line ? "ON LINE" : "ON BATTERY", hex_line);
                ...
                ESP_LOGI(TAG, "📊 TELEMETRY: 0x0D Last Battery Duration = %d min (%d sec) [RAW: %s]", dyn_time_on_batt_min, raw_sec, hex_line);
...
static void request_hid_report_descriptor(usb_device_handle_t dev_hdl, uint8_t intf_num) {
    ...
    err = usb_host_transfer_submit_control(usb_client, ctrl_xfer);
    diag_descriptor_submit_err = err;
    if (err != ESP_OK) {
...
static volatile esp_err_t diag_claim_err = ESP_ERR_INVALID_STATE;
static volatile esp_err_t diag_release_err = ESP_ERR_INVALID_STATE;
static volatile esp_err_t diag_close_err = ESP_ERR_INVALID_STATE;
static volatile uint16_t diag_vid = 0;
static volatile uint16_t diag_pid = 0;
static volatile uint8_t diag_num_interfaces = 0;
static volatile uint8_t diag_interface_class = 0;
...
static void usb_host_client_event_cb(const usb_host_client_event_msg_t *event_msg, void *arg)
{
    ...
    if (client_events_observe_only_diag || device_descriptor_only_diag || config_descriptor_only_diag || interface_claim_only_diag || hid_report_descriptor_minimal_diag) {
        if (event_msg->event == USB_HOST_CLIENT_EVENT_NEW_DEV) {
            diag_new_dev_count++;
            ESP_LOGW(TAG, "USB_DIAG: NEW_DEV #%lu addr=%d observed", (unsigned long)diag_new_dev_count, event_msg->new_dev.address);
            if (device_descriptor_only_diag || config_descriptor_only_diag || interface_claim_only_diag || hid_report_descriptor_minimal_diag) {
                usb_device_handle_t dev_hdl = NULL;
                esp_err_t err = usb_host_device_open(usb_client, event_msg->new_dev.address, &dev_hdl);
                diag_open_err = err;
                ESP_LOGW(TAG, "USB_DEVICE_DESC_ONLY_DIAG: usb_host_device_open -> %s", esp_err_to_name(err));
                if (err == ESP_OK) {
                    const usb_device_desc_t *dev_desc = NULL;
                    err = usb_host_get_device_descriptor(dev_hdl, &dev_desc);
                    diag_device_desc_err = err;
                    ESP_LOGW(TAG, "USB_DEVICE_DESC_ONLY_DIAG: usb_host_get_device_descriptor -> %s", esp_err_to_name(err));
                    if (err == ESP_OK && dev_desc != NULL) {
                        diag_vid = dev_desc->idVendor;
                        diag_pid = dev_desc->idProduct;
                        ESP_LOGW(TAG, "USB_DEVICE_DESC_ONLY_DIAG: VID:PID=%04X:%04X class=0x%02X configs=%d",
                                 dev_desc->idVendor,
                                 dev_desc->idProduct,
                                 dev_desc->bDeviceClass,
                                 dev_desc->bNumConfigurations);
                    }
                    if (config_descriptor_only_diag || interface_claim_only_diag || hid_report_descriptor_minimal_diag) {
                        const usb_config_desc_t *config_desc = NULL;
                        err = usb_host_get_active_config_descriptor(dev_hdl, &config_desc);
                        diag_config_desc_err = err;
                        ESP_LOGW(TAG, "USB_CONFIG_DESC_ONLY_DIAG: usb_host_get_active_config_descriptor -> %s", esp_err_to_name(err));
                        if (err == ESP_OK && config_desc != NULL) {
                            diag_num_interfaces = config_desc->bNumInterfaces;
                            ESP_LOGW(TAG, "USB_CONFIG_DESC_ONLY_DIAG: bNumInterfaces=%d wTotalLength=%d configValue=%d",
                            ...
                            if (intf != NULL) {
                                diag_interface_class = intf->bInterfaceClass;
                                ESP_LOGW(TAG, "USB_CONFIG_DESC_ONLY_DIAG: intf%d class=0x%02X subclass=0x%02X protocol=0x%02X endpoints=%d",
                                         HID_INTERFACE,
                    ...
                    if (interface_claim_only_diag || hid_report_descriptor_minimal_diag) {
                        err = usb_host_interface_claim(usb_client, dev_hdl, HID_INTERFACE, 0);
                        diag_claim_err = err;
                        ESP_LOGW(TAG, "USB_INTERFACE_CLAIM_ONLY_DIAG: usb_host_interface_claim(intf=%d, alt=0) -> %s",
                                 HID_INTERFACE,
                                 esp_err_to_name(err));
                        if (err == ESP_OK && interface_claim_only_diag) {
                            esp_err_t rel_err = usb_host_interface_release(usb_client, dev_hdl, HID_INTERFACE);
                            diag_release_err = rel_err;
                            ESP_LOGW(TAG, "USB_INTERFACE_CLAIM_ONLY_DIAG: usb_host_interface_release(intf=%d) -> %s",
                                     HID_INTERFACE,
                                     esp_err_to_name(rel_err));
                        } else if (err == ESP_OK && hid_report_descriptor_minimal_diag) {
                            ups_device = dev_hdl;
                            ups_connected = true;
                            descriptor_needed = true;
                            descriptor_requested = false;
                            descriptor_complete = false;
                            descriptor_ready_at_ms = (esp_timer_get_time() / 1000) + 30000;
                            descriptor_submit_skipped = false;
                            diag_release_err = ESP_ERR_INVALID_STATE;
                            diag_close_err = ESP_ERR_INVALID_STATE;
                            ESP_LOGW(TAG, "USB_HID_REPORT_DESC_MIN_DIAG: device left open and interface claimed; descriptor request gated for 30s settle");
                        }
                    }
                    if (!hid_report_descriptor_minimal_diag) {
                        err = usb_host_device_close(usb_client, dev_hdl);
                        diag_close_err = err;
                        ESP_LOGW(TAG, "USB_DEVICE_DESC_ONLY_DIAG: usb_host_device_close -> %s", esp_err_to_name(err));
                    }
                }
            } else {
...
void usb_host_hid_report_descriptor_minimal_task(void *arg)
{
    (void)arg;
    ...
    diag_device_desc_err = ESP_ERR_INVALID_STATE;
    diag_config_desc_err = ESP_ERR_INVALID_STATE;
    diag_claim_err = ESP_ERR_INVALID_STATE;
    diag_release_err = ESP_ERR_INVALID_STATE;
    diag_close_err = ESP_ERR_INVALID_STATE;
    diag_vid = 0;
    diag_pid = 0;
    diag_num_interfaces = 0;
    diag_interface_class = 0;
    diag_descriptor_status = -1;
    diag_descriptor_payload_len = -1;
    ...
    while (1) {
        ...
        if (ups_connected && ups_device != NULL && descriptor_needed && !descriptor_requested && !descriptor_submit_skipped) {
            if (descriptor_ready_at_ms > 0 && now_ms < descriptor_ready_at_ms) {
                if ((loop_count % 50) == 0) {
                    usb_diag_heap_checkpoint("settle-before-submit");
                    ESP_LOGW(TAG, "USB_HID_REPORT_DESC_MIN_DIAG: waiting for descriptor settle window, remaining_ms=%lld",
                             (long long)(descriptor_ready_at_ms - now_ms));
                }
            } else {
                bool heap_ok = usb_diag_heap_checkpoint("before-submit");
                if (!heap_ok) {
        ...
        if ((now_ms - last_heartbeat_ms) >= 10000) {
            ...
            ESP_LOGI(TAG, "USB_HID_REPORT_DESC_MIN_DIAG heartbeat: loop=%d errors=%d new_dev=%lu dev_gone=%lu vidpid=%04X:%04X class=0x%02X open=%s devdesc=%s cfg=%s claim=%s submit=%s requested=%d skipped=%d complete=%d cleanup=%d cleanup_skipped=%d desc_status=%d payload=%d raw=%d",
                     loop_count,
                     error_count,
                     ...
                     diag_vid,
                     diag_pid,
                     diag_interface_class,
                     esp_err_to_name(diag_open_err),
                     esp_err_to_name(diag_device_desc_err),
                     esp_err_to_name(diag_config_desc_err),
                     esp_err_to_name(diag_claim_err),
                     esp_err_to_name(diag_descriptor_submit_err),
                     descriptor_requested,
                     descriptor_submit_skipped,
                     descriptor_complete,
                     descriptor_cleanup_done,
                     descriptor_cleanup_skipped,
...
void usb_host_interface_claim_only_task(void *arg)
{
    ...
    while (1) {
        ...
        if ((now_ms - last_heartbeat_ms) >= 10000) {
            last_heartbeat_ms = now_ms;
            ESP_LOGI(TAG, "USB_INTERFACE_CLAIM_ONLY_DIAG heartbeat: loop=%d errors=%d new_dev=%lu dev_gone=%lu vidpid=%04X:%04X intf=%u class=0x%02X open=%s devdesc=%s cfg=%s claim=%s release=%s close=%s",
                     loop_count,
                     error_count,
                     (unsigned long)diag_new_dev_count,
                     (unsigned long)diag_dev_gone_count,
                     ...
                     esp_err_to_name(diag_config_desc_err),
                     esp_err_to_name(diag_claim_err),
                     esp_err_to_name(diag_release_err),
                     esp_err_to_name(diag_close_err));
            if (diag_new_dev_count == 0) {
